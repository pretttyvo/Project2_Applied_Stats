...
```{r logistic ROC}
#Make predictions
pred.log <- as.data.frame(predict(mylogit_3, newdata=testData))
...
abline(a=0, b= 1)
text(x = .40, y = .6,paste("AUC = ", round(auc.train.lda,3), sep = ""))

## Not sure how to create a confusion from a continuous
# Create table for confusion Matrix
predTable.LDA <- tibble(Predicted = pred.lda, Observed = bank.test$y)

#Confusion Matrix
confusionMatrix(predTable.LDA$Predicted, reference = predTable.LDA$Observed)

#Add in a confusion matrix?
predTable <- tibble(Predicted = as.data.frame(pred.lda), Observed = bank.test$y)
