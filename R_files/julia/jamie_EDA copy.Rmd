...
cor(subset(df, select= c("pdays_0","pdays","nr.employed","previous"))) -> df_cortest 
...
```{r}
#plot(yhat.tree,test$y,main="Single Tree with 8 splits",xlab="Predicted",ylab="Test Set Yeses")
#abline(0,1)

mytree<-tree(y ~ education + month,train,minsize=3,mindev=.0001)
yhat.tree<-predict(mytree,newdata=test)
length(yhat.tree)
plot(yhat.tree,test$y,main="Single Tree with Deep Splits",xlab="Predicted",ylab="Test Set Yeses")
abline(0,1)
```

```{r}
#Lets take a look at the predicted surface of our bagged model
predictors<-data.frame(Y=rep(0:300,51),N=rep(0:50,each=301))
bag.full<-randomForest( y ~ education + month,data=test ,
                        mtry=2,importance =TRUE,ntree=100)

pred.surface<-matrix(predict(bag.full,predictors),301,51)
plot3d(education,month,y)
...
dat.train <- trainingsData2
dat.train.x <- dat.train[,c("age","campaign","euribor3m","duration")]
summary(dat.train.x)
