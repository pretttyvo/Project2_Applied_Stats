...
trainingsData <- trainingsData[ , -which(names(trainingsData) %in% c("pdays","ID"))]
...
logit.og <- glm(y ~ job + education + contact + month + 
                ...
                data = trainingsData, family = "binomial") # final logit model - no fitting
summary(logit.og)
#confint(mylogit)
```
STEPWISE
...
fit.pred.lasso <- predict(logit.og, 
                          newx = testData[ , -which(names(testData) %in% c("y","pdays","ID"))], 
                          type = "response")
colnames(testDatax)
(fit.pred.lasso)
testData <- testData[ , -which(names(testData) %in% c("pdays","ID"))]

results.lasso<-prediction(fit.pred.lasso[0:1000], testData$y[0:1000],label.ordering=c("yes","no"))
